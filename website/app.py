...
def get_available_servers():
    ...
    all_servers = mock_db["available_servers"] + mock_db["user_servers"]
...
def add_server():
    ...
    if not any(s['id'] == server_id for s in mock_db['user_servers']):
        ...
        # Find the existing server to return it
        new_server = next((s for s in mock_db['user_servers'] if s['id'] == server_id), None)
    
    return jsonify({"status": "success", "server": new_server})
